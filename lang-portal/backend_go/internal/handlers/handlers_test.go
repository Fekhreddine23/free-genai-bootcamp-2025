...
func setupTestRouter() (*gin.Engine, *Handlers) {
	...
	// Insert test data
	db.Exec("INSERT INTO study_sessions (id, name) VALUES (1, 'Test Session')")
	db.Exec("INSERT INTO words (id, word, definition) VALUES (1, 'Test Word', 'Definition')")
...
func TestReviewWord(t *testing.T) {
	...
	sessionID := 1
	//wordID := 1 // Removed as it is not used

	var count int
	db, _ := models.NewDB("words.test.db")
	db.QueryRow("SELECT COUNT(*) FROM study_sessions WHERE id = ?", sessionID).Scan(&count)
	if count == 0 {
		t.Fatalf("No study session found with ID %d", sessionID)
	}
